...
export const FileUploadField = React.memo(
  <T extends Record<string, unknown>>({
    ...
    const onDrop = useCallback(
      ...
      [field.name, setData, setPreview, preview]
    );
    ...
    // 🔹 Initialize preview from URL (only once)
    useEffect(() => {
      if (
        typeof value === "string" &&
        ...
        >
          ...
          {value ? (
            <div className="flex flex-col items-center justify-center gap-3">
              {/* Image Preview */}
              {preview && (
                <div className="relative flex items-center justify-center w-24 h-24 rounded-lg overflow-hidden border border-gray-600">
                  <Image
                    src={preview}
                    alt="preview"
                    width={100}
              ...
              {!preview && typeof value !== "string" && getFileIcon(value as File)}

              {/* File Name */}
              {typeof value !== "string" && (value as File)?.name && (
