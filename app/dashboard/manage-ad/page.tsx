...
import { Eye, MousePointerClick, MoreVertical } from "lucide-react";
...
const Page = () => {
  ...
  const handleAction = async (action: string, hashId: string) => {
    if (action === "toggle") {
      await toggleAdStatus(hashId);
    } else if (action === "delete") {
        await deleteAd(hashId)
    }
  };
  ...
  return (
    <AdminLayout>
      <Breadcrumb pageName="Manage Ads" createPath="/dashboard/ad-upload" />
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-6">
          {paginatedAds.map((ad) => (
            <div
              key={ad.advt_id}
              className="rounded-2xl border border-gray-800 p-4 shadow-sm bg-neutral-900 hover:bg-neutral-800 transition-all duration-300 relative"
            >
              {/* Options Menu */}
              <div className="absolute top-5 right-5 z-20">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="p-1 rounded-full bg-gray-400 hover:bg-gray-800 transition">
                      <MoreVertical size={20} className="text-red-500" />
                    </button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent
                    align="end"
                  ...
                  >
                    {/* Dynamic Start/Stop */}
                    <DropdownMenuItem
                      onClick={() => handleAction("toggle", ad.hash_id)}
                      className={cn(
                    ...
                    >
                      {ad.advt_status === 1 ? "Stop" : "Start"}
                    </DropdownMenuItem>

                    {/* Delete Option */}
                    <DropdownMenuItem
                      onClick={() => handleAction("delete", ad.hash_id)}
